...
/**
 * Presentation only. page.tsx fetches on the server and passes the post down,
 * so the article text is in the HTML — which is the whole reason this split
 * exists. A 'use client' page would have hidden the body from every crawler and
 * retrieval pipeline that matters.
 */
                ...
                <div className="container-custom relative">
                    <div className="max-w-3xl mx-auto">
                        ...
                        {/* The article itself.
                            ...
                            dangerouslySetInnerHTML is safe here and only here:
                            this HTML was allowlist-sanitised server-side when it
                            was saved (see lambda/shared/utils.ts), so what is in
                            DynamoDB is already the reduced set of tags a post is
                            allowed to contain. Sanitising again at render would
                            imply the stored value is untrusted, which would be
                            the more dangerous belief — it would mean drafts,
                            previews and the API all disagree about what a post
                            is. Trust the write boundary, once.
                            
                            This block replaced ~90 lines of Lorem ipsum and
                            stock images that shipped with the template. The post
                            body was never rendered at all. */}
                        <motion.div
                            initial={{ opacity: 0, y: 30 }}
                            whileInView={{ opacity: 1, y: 0 }}
                            viewport={{ once: true }}
